...
[Setup](#setup)<br>
[Helpers](#helpers)<br>
**Cards** • [Header](#header) • [Weather](#weather-card) • [Favorites](#favorites) • [Climate Overview](#climate-overview) • [Rooms](#rooms) • [Navbar](#navbar)<br>
**Reference** • [Patterns & Tricks](#patterns--tricks) • [Theme Cheat Sheet](#theme-cheat-sheet) • [Status](#status) • [Credits](#credits)

<br>

## About

paper-buttons-row is simply perfect. No other card comes close in flexibility while staying that straightforward and simple. And it loads pretty much instantly, faster than the HA default cards for some mysterious reason. This setup has been running daily for several years now, so here's the documentation, in case someone wants to use it as inspiration.

Two rules shape everything here:

**Cards are dumb, sensors are smart.** Cards never calculate anything. Template sensors prepare every value, every color and every state, and the cards only display attributes. That way the logic lives in one place, and every card that reads it updates for free.

**Nothing is allowed to jump.** No layout shifts, ever. Fixed sizes, states that swap colors instead of sizes, and helpers that always return a value. Both rules come up again and again in [Patterns & Tricks](#patterns--tricks).
...
Light and dark mode follow your profile setting. For the "in sync with the sun" part, use a small automation that calls `frontend.set_theme` with `mode: light` at sunrise and `mode: dark` at sunset. Or set the profile to "Auto" and let the device decide.
...
This dashboard uses [Montserrat](https://fonts.google.com/specimen/Montserrat). It just always looks good. Best to host it locally on your Home Assistant instance, so it loads instantly and doesn't depend on external servers. A late-loading font would also mean reflowing text, and reflowing text means layout shifts.
...
The dashboard leans on a handful of template sensors that do all the thinking. They're documented in their own section: [Helpers](#helpers). If you only want the simple card versions, you can skip them.
...
This is where the "cards are dumb, sensors are smart" rule lives. Each helper is a normal template sensor in your `configuration.yaml` (or a package file). The cards only ever read attributes from them.

One habit worth copying: **every attribute always returns something.** If a source sensor is unavailable, the helper returns `?` instead of nothing. A card that renders `?` keeps its size. A card that renders an empty string collapses and jumps when the value arrives. Same reason for `| default(..., true)` everywhere.

> [!NOTE]
> All entity names below are English placeholders. The file `climate_sensors.yaml` in this repo contains the originals, which use different names. Rename to whatever fits your home.
...
Small detail: when all rooms have the same value (or fewer than two report), both return `none` and no arrows show up anywhere. Without that check, two rooms at exactly 21.0° would randomly fight over the "warmest" arrow.
...
The averages themselves come from two [min/max helpers](https://www.home-assistant.io/integrations/min_max/) (`sensor.temperature_average` and `sensor.humidity_average`, mean over all room sensors), and the window count from a [group helper](https://www.home-assistant.io/integrations/group/) over the window contacts. Those are built in the UI, no YAML needed.
...
The original file also computes a dew point, the temperature spread between rooms, and a ventilation hint that compares absolute humidity inside vs outside and tells you which windows are worth opening. That one is a beast of a template, take a look at `climate_sensors.yaml` if you're curious.
...
The header doesn't watch five media players, it watches one sensor that already picked the interesting speaker and boiled its state down to a single word. The `chip` attribute is a small state machine:
...
A sketch to build from (adjust the picking logic to your players):

```yaml
      - name: Active speaker
        unique_id: active_speaker
        state: "{{ 'on' if states.media_player | selectattr('state', 'eq', 'playing') | list | count > 0 else 'off' }}"
        attributes:
          # pick the player: playing beats paused, last one wins
          speaker: >
            {% set playing = states.media_player | selectattr('state', 'eq', 'playing') | list %}
            {% set paused = states.media_player | selectattr('state', 'eq', 'paused') | list %}
            {% set pick = (playing + paused) | first | default(none) %}
            {{ pick.name if pick is not none else 'none' }}
          title: >
            {% set playing = states.media_player | selectattr('state', 'eq', 'playing') | list %}
            {% set pick = playing | first | default(none) %}
            {{ pick.attributes.media_title | default('') if pick is not none else '' }}
          artwork: >
            {% set playing = states.media_player | selectattr('state', 'eq', 'playing') | list %}
            {% set pick = playing | first | default(none) %}
            {{ pick.attributes.entity_picture | default('none') if pick is not none else 'none' }}
          group: >
            {% set playing = states.media_player | selectattr('state', 'eq', 'playing') | list %}
            {% set pick = playing | first | default(none) %}
            {{ 'yes' if pick is not none and (pick.attributes.group_members | default([]) | count) > 1 else 'no' }}
          chip: >
            {% set playing = states.media_player | selectattr('state', 'eq', 'playing') | list %}
            {% set paused = states.media_player | selectattr('state', 'eq', 'paused') | list %}
            {% set pick = (playing + paused) | first | default(none) %}
            {% if pick is none %} off
            {% else %}
            {% set art = 'art' if pick.attributes.entity_picture is defined else 'plain' %}
            {% set kind = 'tv' if 'tv' in pick.entity_id else ('playing' if pick.state == 'playing' else 'paused') %}
            {{ kind ~ '_' ~ art }}
            {% endif %}
```

Once this exists, the card side becomes a plain lookup table. That's the whole trick.
...
The favorites grid shows tiles conditionally: fans only in fan weather, dinner only when something is planned. All those decisions sit in one sensor, so they can be changed without touching the dashboard.
...
```yaml
      - name: Dashboard buttons
        ...
        state: 'on'
        attributes:
          # 'false' = hide the tile, 'off' = show it idle, anything else = active
          fan_button: >
            {% set hot = states('sensor.temperature_average') | float(0) > 24 %}
            {% set fan = states('fan.bedroom_fan') %}
            {{ fan if fan == 'on' else ('off' if hot else 'false') }}
          dinner_button: >
            {{ 'true' if states('sensor.tandoor_todays_dinner') not in ['unknown', 'unavailable', 'none'] else 'false' }}
```

Note the string `'false'` and the string `'off'` mean different things here: `'false'` hides the tile completely, `'off'` shows it in its quiet look. The tiles read exactly these values in their `state_styles`.
...
Every card comes in two flavors: a **simple version** with standard entities that works almost out of the box, and the **full version** as it runs here, which needs the [helpers](#helpers). The full versions start with a small "what it needs" table, so it's clear which sensors to build first.

All entity names in the examples are generic English placeholders (`light.living_room_lights`, `sensor.climate_bedroom`, ...). Rename them to match your setup. The dashboard as it actually runs, original entity names and all, is in `origami-dashboard.yml`.
...
- The music chip on the right is several buttons stacked into the same grid cells:
  ...
  - the song title, scrolling when longer than 14 characters (see [the marquee pattern](#marquee-text-with-five-states)),
  - the speaker name below it,
  - and a small link badge that pops in when speakers are grouped, and scales away when not (see [morphing with scale(0)](#morphing-with-scale0)).
- One sensor drives the whole chip: `sensor.active_speaker`. Its `chip` attribute is a single word (`playing_art`, `paused_plain`, `tv_art`, `off`, ...) and the card looks that word up in `state_styles` / `state_icons`. All the logic sits in the sensor, see [dumb cards, smart sensors](#dumb-cards-smart-sensors).
...
An [Origami Weather](https://github.com/hazymorning/origami_weather) card that mixes forecast and indoor climate into one view. Inside temperature big on the left, a comfort pill on the right, and a bar that shows inside vs outside at a glance.
...
- **The bar** is the favorite part. It runs from 0-35° with a highlighted comfort band (19-23°) and two markers: a house icon for the indoor average, a tree icon for the balcony. One look tells you whether opening a window helps right now.
- **Bottom row:** "Outside" with the balcony temperature, and the inside/outside difference (`+3` means three degrees warmer inside).
- The card background also tints with the indoor average, through `background_thresholds`. Same color ladder as the pill, so cold days look blue-ish and heat waves glow red. All the fancy effects (rain, clouds, night sky) are switched off here, on purpose. Calm dashboard.
...
Just the weather plus one indoor readout from a plain sensor.
...
| `sensor.climate_home` | Indoor average: `temperature_average`, `humidity_average`, `description`. See [Helpers](#helpers). |
...
- Hiding works with `display: none`, so the grid reflows around the visible tiles and never keeps empty spots.
- None of the tiles decide anything on their own. One helper (`sensor.dashboard_buttons`) holds the visibility logic as attributes, the tiles just read them. Sorting works through plain CSS `order` in `state_styles`, see [active devices float to the front](#active-devices-float-to-the-front).
...
| Entity | Used for |
|---|---|
| `sensor.dashboard_buttons` | The visibility logic: `fan_button` (`false` = hidden, `off` = idle, otherwise active), `dinner_button` (`true` / `false`). See [Helpers](#helpers). |
...
| `sensor.oven` | The oven. State is a short word, `off` hides the tile. |
...
- The **rank arrows** are the fun part: a small up arrow next to the warmest and most humid room, a down arrow next to the coldest and driest. The [peak sensors](#helpers) figure that out, the table just maps `temp_rank` / `humidity_rank` to icons through `state_icons`.
...
- Zebra stripes come from a `repeating-linear-gradient` on the row background, sized to the fixed row height. No extra elements, nothing that can shift. See [zebra stripes for free](#zebra-stripes-for-free).
...
- square: false
  ...
  cards:
    ...
    # The tent — an extra pill row below the table.
    # Yes, there is a tent.
    - type: custom:paper-buttons-row
      styles:
        justify-content: flex-end
        width: max-content
        margin: var(--gap-lg) 0px var(--section-gap) auto
      ...
      base_config:
        layout: icon|state
        entity: sensor.climate_tent
        tap_action:
          action: navigate
          navigation_path: '#popup_tent'
        styles:
          button:
            padding: 0px
            justify-content: center
        ...
        - icon: mdi:circle
          ...
          styles:
            ...
            icon:
              opacity: 1
              color: >-
                {{ state_attr("sensor.climate_tent", "color_code")
                }}
        - icon: mdi:thermometer
          state:
            attribute: temperature
...
- An **invisible button stretched across the whole card** does the heavy lifting. It's bound to the room's light group, sits on top of everything (`z-index: 3`), tints the entire card with `--accent-light-on-background` while lights are on, and catches the tap, which opens the room popup.
...
- Why paper-buttons-row instead of a normal grid of tile cards? One card instead of six per room, one tap target with a clean tint, and everything styles through the same theme variables.
...
| `sensor.climate_<room>` | `temperature`, `humidity`, `color_code` for the bottom line. See [Helpers](#helpers). |
...
- The `styles` block wires the card into the theme: same radius, shadow and backdrop blur as everything else, ripple off, floating on mobile.
...
Three routes and one badge. Works with any light group.
...
routes:
  - icon: mdi:lightbulb-variant
    ...
    badge:
      ...
      show: "[[[return states['sensor.lights_on_count']?.state > 0;]]]"
      count: "[[[return states['sensor.lights_on_count'].state;]]]"
  - icon: mdi:star
    tap_action:
      action: navigate
      navigation_path: '#popup_favorites'
...
The two counters are one-liners:
...
If your TVs report as `playing` too, filter the speaker counter down to a fixed list of players.
...
The same handful of ideas repeats through every card. Once these click, the big YAML blocks above stop looking scary.
...
### Dumb cards, smart sensors

Cards never calculate. A template sensor prepares one word (or number, or color) per visual state, and the card maps that word to styles through `state_styles` / `state_icons`. The logic lives in one place, every card reading the sensor updates for free, and the card YAML stays a lookup table you can actually read.
...
No conditions in the card, no nested templates. Change the logic in the sensor and every card follows.

</details>

### No layout shifts

The rule behind half the odd choices here. Habits that make it work:
...
- **Elements that come and go either reflow on purpose** (`display: none` in the favorites grid, the grid closes the gap) **or keep their box** (`transform: scale(0)`, see below).
...
### Active devices float to the front
...
One thing to know: the `rgb-*` variants of these variables want bare color triplets (`136, 163, 126`), not hex values.
...
|---|---|
| `accent-color` | The sage green. Carries most of the theme's personality. |
...
| `fill-subtle` / `fill-strong` / `fill-loud` | Neutral surfaces as 6 / 12 / 30 % tints of the text color. They adapt to both modes for free. |
...
- [x] Climate helper templates
- [ ] Remaining helpers in full (speaker picking, favorites logic)
...
- [Paper Buttons Row](https://github.com/jcwillox/lovelace-paper-buttons-row) by jcwillox — the card that carries this whole dashboard
- [Origami Weather](https://github.com/hazymorning/origami_weather)
- [Navbar Card](https://github.com/joseluis9595/lovelace-navbar-card)
- [Kiosk Mode](https://github.com/NemesisRE/kiosk-mode)
- [Montserrat](https://fonts.google.com/specimen/Montserrat)
- The Home Assistant community, for years of ideas to borrow
