...
<details>
<summary><b>1. Theme</b></summary>

<br>

The theme is what makes everything look right, so it comes first. Install it through HACS and you'll get updates like for any other HACS thing.

[![Open your Home Assistant instance and open this repository inside HACS.](https://my.home-assistant.io/badges/hacs_repository.svg)](https://my.home-assistant.io/redirect/hacs_repository/?owner=hazymorning&repository=origami&category=theme)

1. Tap the blue button. It opens this repository right inside your HACS. Confirm with **Add**, then **Download**.
2. Reload themes: **Developer tools → Actions → `Frontend: Reload themes`**. A restart works too.
3. Open your **profile** (your name, bottom left) and pick **Origami** under Theme. Set it to "Auto" if you want it to follow your device's dark mode.

<details>
<summary>Button doesn't work, or you'd rather do it by hand?</summary>

<br>

**HACS, manually:** open HACS → three dots top right → **Custom repositories** → paste `https://github.com/hazymorning/origami`, type **Theme** → **Add**. Then find "Origami" in HACS and download it.

**Without HACS:** copy `themes/origami.yaml` from this repo into the `themes` folder of your Home Assistant config, then reload themes.

**No themes folder yet?** Add this to your `configuration.yaml` once, create an empty `themes` folder next to it, and restart:
...
<details>
<summary><b>2. Cards</b></summary>

<br>

Everything else comes from HACS as well:
